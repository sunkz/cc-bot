...
`Claude Code` 通知助手 — 通过 macOS `系统通知` 和 `Telegram Bot` 实时推送执行状态，不用再盯着 `CLI` 等结果。
...
- **ACP Proxy** — 代理 ACP 协议，拦截权限确认请求并通知
...
ACP Server ──Proxy──▶ ACPProxy ──▶ HookServer ──▶ 系统通知 / Telegram
```

1. **HookServer** — 在本地 `62400` 端口启动 TCP 服务，接收 Claude Code Hook 发来的 JSON 请求
2. **CCGUIWatcher** — 监听 CC GUI 插件的文件系统 IPC 目录，检测权限请求文件
3. **ACPProxy** — Node.js 脚本，包裹 ACP Server 命令，拦截 `request_permission` JSON-RPC 消息
...
点击菜单栏中的 **安装** 按钮，CCBot 会生成代理脚本 `~/.claude/hooks/cc-bot-acp-proxy.mjs`。

在 `~/.jetbrains/acp.json` 中配置使用代理：

```json
{
  "command": "~/.claude/hooks/cc-bot-acp-proxy.mjs",
  "args": ["/opt/homebrew/bin/npx", "-y", "@anthropic-ai/claude-agent-acp"]
}
```
