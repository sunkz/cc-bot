...
`Claude Code` / `Codex` / `Paseo` 通知助手 — 通过 macOS `系统通知` 和 `Telegram Bot` 实时推送执行状态，不用再盯着 `CLI` 等结果。
...
- **Paseo 监听** — 通过 WebSocket 连接 Paseo daemon，监听 agent 权限请求和完成事件
...
Paseo Daemon ──WebSocket──▶ PaseoWatcher ──▶ 系统通知 / Telegram
```
...
4. **PaseoWatcher** — 通过 WebSocket 连接 Paseo daemon(:6767)，监听 permission_requested 和 attention_required 事件
...
默认开启。自动连接本地 Paseo daemon（`ws://127.0.0.1:6767/ws`），监听 agent 权限请求和完成事件。需先安装 [Paseo](https://github.com/getpaseo/paseo) 并启动 daemon。
...
- **URLSessionWebSocketTask** — WebSocket 连接
