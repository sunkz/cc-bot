            ...
            do {
                ...
                if let http = resp as? HTTPURLResponse {
                    ...
                    if http.statusCode >= 400, http.statusCode < 500 {
                        log.error("event=telegram_send status=\(http.statusCode) retry=false attempt=\(attempt + 1) reason=\(result.description ?? "unknown error")")
                        return
                    }
                    log.warning("event=telegram_send status=\(http.statusCode) retry=true attempt=\(attempt + 1)/3 reason=\(result.description ?? "unknown error")")
                }
